...
class Uniform:
    ...
    def locate_variable(self, program_ref, variable_name: str):
        match self.data_type:
            ...
            case DataType.shadow:
                ...
                self.variable_ref["projectionMatrix"] = GL.glGetUniformLocation(
                    program_ref, f"{variable_name}.projectionMatrix"
                )
                self.variable_ref["viewMatrix"] = GL.glGetUniformLocation(
                    program_ref, f"{variable_name}.viewMatrix"
                )
                self.variable_ref["depthTextureSampler"] = GL.glGetUniformLocation(
                    program_ref, f"{variable_name}.depthTextureSampler"
                )
                self.variable_ref["strength"] = GL.glGetUniformLocation(
                    program_ref, f"{variable_name}.strength"
                )
                self.variable_ref["bias"] = GL.glGetUniformLocation(
    ...
    def upload_data(self):
        ...
        match self.data_type:
            ...
            case DataType.vec3:
                GL.glUniform3f(self.variable_ref, *self.data[0:3])
            case DataType.vec4:
            ...
            case DataType.light:
                GL.glUniform1i(self.variable_ref["lightType"], self.data.light_type)
                ...
                GL.glUniform3f(self.variable_ref["attenuation"], *self.data.attenuation)
            case DataType.shadow:
                ...
                GL.glBindTexture(GL.GL_TEXTURE_2D, texture_object_ref)
                GL.glUniform1i(
                    self.variable_ref["depthTextureSampler"], texture_unit_ref
                )

                GL.glUniform1f(self.variable_ref["strength"], self.data.strength)
                GL.glUniform1f(self.variable_ref["bias"], self.data.bias)
            case _:
                raise Exception(f"Unknown type {self.data_type}")
