...
class CylindricalGeometry(ParametricGeometry):
    ...
    def __init__(self, radius_top=1, radius_bottom=1, height=1, radial_segments=32,
        ...
        def s(u, v):
            return [(v * radius_top + (1 - v) * radius_bottom) * sin(u), height * (v - 0.5),
        ...
        if closed_top:
            ...
            transform = Matrix.make_translation(
                0, height / 2, 0) @ Matrix.make_rotation_y(-pi / 2) @ Matrix.make_rotation_x(-pi / 2)
